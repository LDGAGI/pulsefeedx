...
const getDefaultFromEmail = () => {
  ...
  if (process.env.NODE_ENV === 'development') {
    return 'PulseFeedX <onboarding@resend.dev>';
  }
  ...
  // 4. 使用配置的域名和应用名称
  const appName = process.env.NEXT_PUBLIC_APP_NAME || 'PulseFeedX';
...
export async function sendVerificationEmail(email: string, token: string) {
  ...
  return sendEmail({
    to: email,
    subject: 'Verify your email - PulseFeedX',
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
        <h1 style="color: #333;">Welcome to PulseFeedX!</h1>
        ...
        <p style="color: #999; font-size: 14px; margin-top: 30px;">
          If you didn't sign up for PulseFeedX, you can safely ignore this email.
        </p>
      </div>
    `,
  });
...
export async function sendPasswordResetEmail(email: string, token: string) {
  ...
  return sendEmail({
    to: email,
    subject: 'Reset your password - PulseFeedX',
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
...
export async function sendWelcomeEmail(email: string, name?: string) {
  return sendEmail({
    to: email,
    subject: 'Welcome to PulseFeedX!',
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
        <h1 style="color: #333;">Welcome to PulseFeedX${name ? ', ' + name : ''}!</h1>
...
export async function sendPurchaseEmail(email: string, orderDetails: any) {
  ...
  return sendEmail({
    to: email,
    subject: 'Purchase Confirmation - PulseFeedX',
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <p style="color: #999; font-size: 14px; margin-top: 30px;">
          Thank you for choosing PulseFeedX!
        </p>
      </div>
    `,
  });
...
export async function sendSubscriptionExpiryReminder(email: string, daysRemaining: number) {
  return sendEmail({
    to: email,
    subject: `Your subscription expires in ${daysRemaining} days - PulseFeedX`,
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
        <h1 style="color: #333;">Subscription Expiry Reminder</h1>
        <p>Your PulseFeedX subscription will expire in <strong>${daysRemaining} days</strong>.</p>
...
export async function sendLowCreditsNotification(email: string, remainingCredits: number) {
  return sendEmail({
    to: email,
    subject: 'Low Credits Alert - PulseFeedX',
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
